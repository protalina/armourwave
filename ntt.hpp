...
constexpr auto thou = field::zq_t(17);
...
// Compile-time compute powers of thou, used for polynomial evaluation ( NTT )
...
compute_ntt_thou()
{
  ...
  for (size_t i = 0; i < N / 2; i++) {
    res[i] = thou ^ bit_rev<LOG2N - 1>(i);
  }

  return res;
}

// Precomputed constants ( powers of thou ), used for computing NTT form of
// degree-255 polynomial
constexpr std::array<field::zq_t, N / 2> NTT_thou_EXP = compute_ntt_thou();

// Compile-time compute negated powers of thou, used for polynomial interpolation (
// iNTT )
consteval std::array<field::zq_t, N / 2>
compute_intt_thou()
{
  ...
  for (size_t i = 0; i < N / 2; i++) {
    res[i] = -NTT_thou_EXP[i];
  }

  return res;
}

// Precomputed constants ( negated powers of thou ), used for computing coefficient
// form of degree-255 polynomial using inverse NTT
constexpr std::array<field::zq_t, N / 2> INTT_thou_EXP = compute_intt_thou();

// Compile-time compute powers of thou, used for multiplication of two degree-255
...
compute_mul_thou()
{
  ...
  for (size_t i = 0; i < N / 2; i++) {
    res[i] = thou ^ ((bit_rev<LOG2N - 1>(i) << 1) ^ 1);
  }

  return res;
}

// Precomputed constants ( powers of thou ), used when multiplying two degree-255
// polynomials in NTT domain.
constexpr std::array<field::zq_t, N / 2> POLY_MUL_thou_EXP = compute_mul_thou();
...
ntt(std::span<field::zq_t, N> poly)
{
  for (size_t l = LOG2N - 1; l >= 1; l--) {
    ...
    for (size_t start = 0; start < poly.size(); start += lenx2) {
      ...
      //
      // thou ^ bit_rev<LOG2N - 1>(k_now)
      //
      // This is how these constants are generated !
      const field::zq_t thou_exp = NTT_thou_EXP[k_now];
...
intt(std::span<field::zq_t, N> poly)
{
  for (size_t l = 1; l < LOG2N; l++) {
    ...
    for (size_t start = 0; start < poly.size(); start += lenx2) {
      ...
      //
      // -NTT_thou_EXP[k_now]
      const field::zq_t neg_thou_exp = INTT_thou_EXP[k_now];
...
//
// h = f * g mod X ^ 2 − thou ^ (2 * br<7>(i) + 1) | i ∈ [0, 128)
//
...
)
{
  ...
  for (size_t i = 0; i < cnt; i++) {
    ...
    basemul(poly_t(f.subspan(off, 2)), poly_t(g.subspan(off, 2)), mut_poly_t(h.subspan(off, 2)), POLY_MUL_thou_EXP[i]);
  }
}
